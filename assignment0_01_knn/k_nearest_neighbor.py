...
class KNearestNeighbor:
    ...
    def compute_distances_no_loops(self, X):
        ...
        #########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        columns = np.sum(X**2, axis=1).reshape(-1, 1)
        rows = np.sum(self.X_train**2, axis=1)
        matrix_prod = 2 * np.dot(X, self.X_train.T)
        diff = columns - matrix_prod + rows
        dists = np.sqrt(diff)
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        return dists
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            nearest_indx = np.argsort(dists[i])[:k]

            closest_y = self.y_train[nearest_indx]
